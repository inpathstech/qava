(function () {
  function startNeedStrategyChart(plot) {
    ...
    const TARGETS = [1.5, 3.6, 9.2];
    const XS = [188, 274, 360];
    const Y0 = 186.8;
    const Y1 = [169.2, 126.4, 30.8];
    const STAGGER = 167;
    const RISE = 1600;
    const CYCLE = 3200;
    const BAND_IN = 667;
    ...
    svg.innerHTML = `
      ...
      <line x1="72" y1="67.5" x2="384" y2="67.5" stroke="#e5e7eb" stroke-width="1" stroke-dasharray="3 3"/>
      <line x1="72" y1="130" x2="384" y2="130" stroke="#e5e7eb" stroke-width="1" stroke-dasharray="3 3"/>
      <rect x="67.6" y="10.4" width="9.2" height="51.8" rx="2.1" fill="#fff" stroke="#e5e7eb" stroke-width="0.9"/>
    ...
    const midPill = svg.querySelector(".mid-pill");
    const line = svg.querySelector(".line");
    const dots = XS.map((x, i) => {
      const stem = document.createElementNS(NS, "line");
      stem.setAttribute("stroke", "#e5e7eb");
      stem.setAttribute("stroke-width", "1");
      stem.setAttribute("stroke-dasharray", "2.5 3");
      ...
      t.setAttribute("class", "qava-need-chart-score");
    ...
    function ease(t) {
      ...
      const u = (t - 0.52) / 0.48;
      return 0.34 + 0.66 * (1 - Math.pow(1 - u, 2));
    }

    function paint(nowOffset) {
      const t = reduce ? CYCLE : nowOffset;
      const bandP = Math.max(0, Math.min(1, (t - BAND_IN) / BAND_DUR));
      const bandE = 1 - Math.pow(1 - bandP, 2);
      band.setAttribute("opacity", String(bandE));
      midPill.setAttribute("opacity", String(bandE));

      const pts = dots.map((dot, i) => {
        const local = Math.max(0, Math.min(1, (t - i * STAGGER) / RISE));
        const p = reduce ? 1 : ease(local);
        const y = Y0 + (dot.y1 - Y0) * p;
        dot.stem.setAttribute("y1", y + 6);
        dot.stem.setAttribute("y2", Y0);
        dot.glow.setAttribute("cy", y);
        dot.c.setAttribute("cy", y);
        dot.t.setAttribute("x", dot.x + (i === 1 ? -11 : 0));
        dot.t.setAttribute("y", y - (i === 1 ? 15 : 13));
        dot.t.textContent = (dot.target * p).toFixed(1);
        return dot.x + "," + y;
      });
          ...
          if (logosAnchorPoint && universityLogosRow) {
            ...
            if (!doc.getElementById("qava-calc-section")) {
              ...
              const stories = [
                ...
                { quote: "Matched in a day. Scoped cleanly. Paid on time. I’ll keep coming back — every project so far has been worth the effort and well organized.", logo: "./Landing%20Page%20Trusted%20by/HBS.png", h: 32, name: "Harvard Business School", sub: "Class of 2024" },
                { quote: "Finally, project work that respects MBA-level thinking — not busywork. The scope was clear from day one and the client valued my strategic input.", logo: "./Landing%20Page%20Trusted%20by/Haas.png", h: 32, name: "Berkeley Haas", sub: "Class of 2025" },
                { quote: "I shipped a pricing project that the client still uses. That’s the bar I now hold every engagement to, and it’s made me a sharper strategist.", logo: "./Landing%20Page%20Trusted%20by/Standford.png", h: 32, name: "Stanford GSB", sub: "Class of 2023" },
                { quote: "Clear scope, sharp clients, and work I can put on a resume with pride. Every project felt like real consulting, not a filler internship task.", logo: "./Landing%20Page%20Trusted%20by/MIT.png", h: 32, name: "MIT Sloan", sub: "Class of 2024" },
                { quote: "Better than cold outreach. Better briefs. Better outcomes. Simple as that — I spent less time chasing leads and more time doing real work.", logo: "./Landing%20Page%20Trusted%20by/Stern.png", h: 32, name: "NYU Stern", sub: "Class of 2025" },
                { quote: "Remote, high-trust projects with founders who actually wanted my input. It felt like being a true partner, not just an extra pair of hands.", logo: "./Landing%20Page%20Trusted%20by/Said.png", h: 32, name: "Oxford Saïd", sub: "Class of 2024" },
                ...
                { quote: "Applied Friday, kicked off Monday. The platform just moves — no drawn-out interviews, no waiting around, just straight into meaningful work.", logo: "./Landing%20Page%20Trusted%20by/Kellogg.png", h: 32, name: "Kellogg", sub: "Class of 2025" },
                { quote: "Quality over volume. Every listing felt worth opening, and the clients I matched with were serious about the problems they needed solved.", logo: "./Landing%20Page%20Trusted%20by/Wharton.png", h: 32, name: "Wharton", sub: "Class of 2024" },
